...
static int Video_ChangeVideoMode(t_video_driver* driver, int w, int h, bool fullscreen, int refresh_rate, bool fatal)
{
    ...
    if (fullscreen) {
		//display_flags |= ALLEGRO_FULLSCREEN;  // This wasn't working on El Capitan
...
void    Video_Setup_State(void)
{
    ...
    // TOMMOUSE //
    // There is also code in inputs.c/Inputs_SetMouseCursor()
...
void    Screen_Restore_from_Next_Buffer(void)
{
	...
	//al_draw_bitmap(screenbuffer_next, 0, 0, ALLEGRO_FLIP_VERTICAL);
}
